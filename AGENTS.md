...
8. **保真迁移**:迁移默认只做 ROS1 Noetic 到 ROS2 Jazzy 及其运行平台所必需的适配，包括构建系统、ROS API、消息类型限定、QoS、TF、Gazebo 后端和资源路径。除这些必要适配外，必须保留 legacy 的算法、类与函数职责、状态机、数据流、参数及默认值、topic/service/message 语义、坐标约定和输出格式；不得用简化实现、替代算法或演示代码代替原实现。
9. **文件同名对应**:功能实现文件必须保留 legacy 的原文件名和相对目录，便于逐文件 diff；不得以新命名文件合并、拆分或替代原实现。仅 ROS2 工具链或本项目明确规范允许例外，例如消息文件使用 PascalCase、launch 改为 `.launch.py`、ROS 包名改为小写；每个例外必须在阶段文档和文件头注明原路径、映射关系及原因。可以新增独立的 ROS2 适配文件，但不能借此删除或隐藏原功能实现。
10. **禁止未经批准删改功能**:发现 legacy 代码暂时无法迁移时，必须先记录阻塞点并继续寻找等价 ROS2 方案。任何文件、类、函数、分支、参数、订阅发布接口或算法步骤的删除、降级、合并、拆分、改名及行为变更，都必须在实施前获得用户明确批准，并在独立文档中列出差异；未获批准不得实施，也不得将对应阶段标记为完成。
11. **迁移审查标准**:编译通过和链路可运行只证明 ROS2 适配可执行，不等于迁移完成。阶段验收还必须提供 legacy/src 文件映射、缺失项检查和关键逻辑对照结果，确认没有未经批准的简化或行为改变。

## 迁移工作流

1. **迁移盘点**:实施前列出 legacy 包内文件、类、主要函数、参数和 ROS 接口，并建立 legacy/src 一一映射；未映射项必须说明原因且获得用户批准。
2. **迁移可追溯**:迁移到 `src/` 的每个文件顶部,注明原 ROS1 参考文件路径(`legacy/...`),便于审核对照。
3. 按 `docs/migration-plan.md` 的阶段顺序迁移，并参考对应的独立阶段规划文档。
4. 先保持原文件和原逻辑，仅在对应位置替换 ROS1 API；完成后检查同名文件、类、函数、参数、topic 和关键分支是否遗漏。
5. 改完一个包,在容器内 `colcon build --packages-select <pkg>` 验证。
6. 在容器内完成行为与数据流验证，并将结果及 legacy/src 对照结论记录到 `docs/experiments/`。
7. 验证通过后 commit。
